...
# KustDesk Server 替代端口一键部署脚本
# 作者: Kayungko
# 版本: v1.0.0
# 说明: 使用8014-8019端口避免21114-21119端口被封禁
...
start_api() {
    ...
    docker run -d \
        ...
        -e TZ=Asia/Shanghai \
        -e RUSTDESK_API_RUSTDESK_ID_SERVER=192.168.1.66:8016 \
        -e RUSTDESK_API_RUSTDESK_RELAY_SERVER=192.168.1.66:8017 \
        -e RUSTDESK_API_RUSTDESK_API_SERVER=http://192.168.1.66:8014 \
...
# 启动Server服务
start_server() {
    log_info "启动 Server 服务 (端口8015-8019)..."
...
show_info() {
    ...
    echo ""
    echo "=========================================="
    echo "🎉 KustDesk Server 部署完成！(替代端口)"
    echo "=========================================="
    echo ""
    echo "📋 连接信息："
    echo "  ID服务器: ${SERVER_IP}:8016"
    echo "  中继服务器: ${SERVER_IP}:8017"
    echo "  Web管理界面: http://${SERVER_IP}:8014"
    echo "  Web客户端: http://${SERVER_IP}:8018"
    echo "  Web客户端HTTPS: https://${SERVER_IP}:8019"
    echo ""
    echo "🔑 公钥:"
    echo "  ${PUBLIC_KEY}"
    echo ""
...
# 主函数
main() {
    echo "🚀 KustDesk Server 替代端口一键部署脚本"
    echo "=========================================="
    echo ""
